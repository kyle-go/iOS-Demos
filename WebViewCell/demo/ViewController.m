...
- (void)viewDidLoad
{
    ...
    NSString *a3 = @"@田爱娜刚发现粉丝快1万了，为了纪念下，特提供200元的乐视点卡作为奖品赠送给第一万的粉丝。PS：前晚用上乐视盒子了，再装个“泰捷视频”，完美之极。等有钱了再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配再配个乐视超级电视和智能遥控就更完美了。 --- END";
